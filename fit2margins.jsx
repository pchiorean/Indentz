...
var spread = doc.selection[0].parentPage.parent;
...
if (doc.selection.length != 0) {
    doc.selection[0].geometricBounds = pageSafeArea(spread.index)
} else {
    // ***TODO*** Check if on pasteboard
    // alert("Please select an object and try again.")
}

// Function to calculate safe area coordinates from page/spread margin size
...
function pageSafeArea(s) {
    ...
    var spreadPages = doc.selection[0].parentPage.parent.pages;
    var curPage = doc.spreads[s].pages[0];
    // alert("spread: " + s + "\rpage in spread: " + curPage.index + "/" + (spreadPages.length - 1));
