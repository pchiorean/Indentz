...
function QRFile() {
	var file = File(doc.filePath + "/" + 
		doc.name.substr(0, doc.name.lastIndexOf(".")) + "_QR.indd");
	var fileq = app.documents.add();
	var page = fileq.pages[0];
	var infoLayer = MakeInfoLayer(fileq);
	// QR label frame
	...
	fileq.documentPreferences.pageWidth = page.bounds[3] - page.bounds[1];
	fileq.documentPreferences.pageHeight = page.bounds[2] - page.bounds[0];
	QR.ungroup();
	fileq.save(file); // fileq.close();
}
