...
function grid() {
	var guidesLayer, hwLayer, page, tgBounds, tgSize, MG;
	var guidesLayerName = 'guides';
	var hwLayerName = 'HW';
	var HW_PCT = 10;
	var MG_PCT = Number(File($.fileName).name.match(/\d+/) || 5);
	...
	for (var i = 0; i < doc.pages.length; i++) {
		...
		page.marginPreferences.properties = {
			...
			bottom: (page.bounds[2] - tgBounds[2]) + tgSize.height * Number(HW_PCT) / 100 + MG,
		...
		// Common guides
		addGuide(page, guidesLayer, 'h', tgBounds[0] + tgSize.height * (1 - Number(HW_PCT) / 100) / 2, 'middle', 'x');
		addGuide(page, guidesLayer, 'v', tgBounds[1] + tgSize.width / 2, 'middle', 'x');
		addGuide(page, hwLayer, 'h', tgBounds[0] + tgSize.height * (1 - Number(HW_PCT) / 100), 'hw', 's');
	}
